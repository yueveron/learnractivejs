...
document.addEventListener("DOMContentLoaded", function () {
  ...
  ProductList.on({
    add: function (event) {
      // event.resolve() -> 解析获取实例 keypath name, return : products.index(products.0, products.1,...)
      this.set(event.resolve() + '.in_cart', true);
      // event.get() - > 获得 keypath data
      Cart.push('products', event.get()); // 将单个商品data , push 入 cart's data array
    },

    discard: function (event) {
      
      this.set(event.resolve() + '.in_cart', false);
      //将在 cart's data array 的对应商品data，移出数组
    ...
    manualproxy : function(event, message) {
      console.log(message)
    }
  });
...
/* data of inner-shopcar's products */
